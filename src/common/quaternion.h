...
extern "C" {
    ...
    #define quat_lensq(q)           (qw(q)*qw(q) + qx(q)*qx(q) + qy(q)*qy(q) + qz(q)*qz(q))

    /* sqrt(qw*qw + qx*qx + qy*qy + qz*qz) */
    #define quat_len(q)             r_sqrt(quat_lensq(q))

    /* rw,rx,ry,rz = aw+bw, ax+bx, ay+by, az+bz */
    #define quat_add(r, a, b)       do {qw(r) = qw(a)+qw(b); \
                                        qx(r) = qx(a)+qx(b); \
                                        qy(r) = qy(a)+qy(b); \
                                        qz(r) = qz(a)+qz(b); } while ( 0 )

    /* rw,rx,ry,rz = aw-bw, ax-bx, ay-by, az-bz */
    #define quat_sub(r, a, b)       do {qw(r) = qw(a)-qw(b); \
                                        qx(r) = qx(a)-qx(b); \
                                        qy(r) = qy(a)-qy(b); \
                                        qz(r) = qz(a)-qz(b);} while ( 0 )
     ...
     **/
    #define quat_mul(r, a, b)       do {qw(r) = qw(a)*qw(b) - qx(a)*qx(b) - qy(a)*qy(b) - qz(a)*qz(b); \
                                        qx(r) = qx(a)*qw(b) + qx(b)*qw(a) + qy(b)*qz(a) - qy(a)*qz(b); \
                                        qy(r) = qy(a)*qw(b) + qy(b)*qw(a) + qx(a)*qz(b) - qx(b)*qz(a); \
                                        qz(r) = qz(a)*qw(b) + qz(b)*qw(a) + qx(b)*qy(a) - qx(a)*qy(b); } while ( 0 )

    /* rw,rx,ry,rz = -qw, -qx, -qy, -qz */
    #define quat_neg(r, q)          do { qw(r) = -qw(q); qx(r) = -qx(q); qy(r) = -qy(q); qz(r) = -qz(q); } while ( 0 )

    /* rw,rx,ry,rz = qw, -qx, -qy, -qz */
    #define quat_conjugate(r, q)    do { qw(r) = qw(q); qx(r) = -qx(q); qy(r) = -qy(q); qz(r) = -qz(q); } while ( 0 )

    /* aw*bw + ax*bx + ay*by + az*bz */
    #define quat_dot(a, b)          (qw(a)*qw(b) + qx(a)*qx(b) + qy(a)*qy(b) + qz(a)*qz(b))

    /* qw,qx,qy,qz = qw*(length/qlen), qx*(length/qlen), qy*(length/qlen), qz*(length/qlen) */
